...
- `POST /api/wardrobe/:id/sustainability` -> looks up item.sourceName in sustainability_cache first; on miss, does a lookup, writes to cache + onto the item, returns it. Must explicitly avoid inventing unsourced claims — fall back to a labeled generic per-category estimate if nothing is found. **Scope change:** originally spec'd as web-search-grounded, but Gemini's search tool requires billing on the API key, which we don't have — this now answers from the model's training knowledge only, with an explicit low-confidence fallback instead of citations. `sustainabilitySources` is always `[]`. Swap real grounding back in if a billing-enabled key shows up later (see git history on sustainability.js).
